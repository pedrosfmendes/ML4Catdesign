...
def change_dataset_factors(var, factor, datapoints, seed):
    # defining the boundary values of the features
    metal_conc_min = 0.3
    #metal_conc_min = metal_conc_min if var is not 'metal' else metal_conc_min*factor
    metal_conc_max = 70
    metal_conc_max = metal_conc_max if var is not 'metal' else metal_conc_max*factor
    metal_conc_delta = metal_conc_max - metal_conc_min

    acid_conc_min = 114
    #acid_conc_min = acid_conc_min if var is not 'acid' else acid_conc_min*factor
    acid_conc_max = 1458
    acid_conc_max = acid_conc_max if var is not 'acid' else acid_conc_max*factor
    acid_conc_delta = acid_conc_max - acid_conc_min

    gamma_min = 0.2
    gamma_max = 0.84
    gamma_max = gamma_max if var is not 'gamma' else gamma_max*factor
    gamma_delta = gamma_max - gamma_min

    epsilon_min = 37/100000
    epsilon_max = 45/10000
    epsilon_max = epsilon_max if var is not 'epsilon' else epsilon_max*factor
...
directory = os.path.join('.', 'results', 'design')
if not os.path.exists(directory):
    os.makedirs(directory)

factors = [1/10., 1/5., 1/2., 1., 2., 5., 10.]
variable = ['metal', 'acid']
data = dict()

for var in variable:
    for f in factors:
        data[(var, f)] = change_dataset_factors(var, f, 100000, 0)
        
with open(os.path.join(directory, 'DesignSpace_100k.pickle'), 'wb') as f:
     pickle.dump(data, f)          
...
for key in data.keys():
    ...
    with open(os.path.join(directory,'results_design.pickle'), 'wb') as f:
